...
spider.route("bluegrasslyrics.com", "/node/*", function(win, $) {
  ...
  if (song.version)
    song.fullName = song.version + ' - ';
  else if (song.author)
    song.fullName = song.author + ' - ';
  if (!song.fullName)
    song.fullName = 'Unknown - ';
  song.fullName = song.fullName + song.title;

  //song.slug = slugs(song.fullName);
